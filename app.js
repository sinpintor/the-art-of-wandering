 ...
 * Author: Wade & Cloudy
 * Note: 包含完整 368 鄉鎮資料、36 張牌卡風味庫、JPG 海報輸出
 */

// 1. 台灣行政區資料庫 (完整 368 鄉鎮)
...
// 3. 隨機沿途風味資料庫 (依據第 2 張牌產生)
// 編輯備註：若要修改風味文案，請直接修改下方引號內的文字
const tasteLibrary = {
...
// 5. 全域變數與 API
// 編輯備註：請將下方的 URL 替換為你最新的 Google Apps Script 部署網址
        ...
        listEl.innerHTML = cloudData.length === 0 ? `<div class="state-message">公路日記還是空白的。</div>` :
            [...cloudData].reverse().map((log, i, arr) => `
                <div class="journal-card">
                    ...
                    <div style="font-size:0.5rem; letter-spacing:0.25em; color:#666; margin-bottom:12px;">${formatToDateOnly(log.date)} / ${log.combo}</div>
...
function drawFate(manualLoc = null) {
    ...
    const timer = setInterval(() => {
        ...
        if (i > 25) {
            ...
            const m_f = tasteLibrary[id2]; // 使用 id2 對應的隨機風味
            ...
            currentRecord = { 
                ...
                color: s1.color, date: new Date().toISOString() 
            };
        }
    }, 60);
}
